...
这是一个可复现的批量图像预处理工具。它能够增强图像、定位高亮参考区域、估计主体方向、围绕检测中心对齐图像，并按指定尺寸和位置裁剪后续分析所需的区域。

项目同时提供 Tkinter 图形界面、命令行入口和可复用的 Python API。

## 效果演示

以下示例完全由程序生成，不包含真实实验图像或隐私数据。
...
该方法具有确定性和可解释性，但并不是适用于所有图像的通用配准算法。
...
仓库不包含真实实验数据。README中的图片均由程序生成。使用者在处理或公开第三方图像前，应确认已经获得相应授权。
