...
A Python tool for batch image enhancement, alignment and cropping. It enhances contrast, locates a bright reference region, estimates the main direction and crops an area around the detected center.

The project provides a Tkinter desktop interface, a command-line interface and a reusable Python package.

## Demo

The example images are generated by `examples/generate_demo.py`.
...
This approach works best when the reference spot is clear and the main structure has a stable direction. It is not designed for general-purpose image registration.
...
Regenerate the README example images with:
